// todo: write that all the credit for this piece of code
// goes to the owner of the follwing github account
// https://github.com/svrebos/TabToChordPro
module.exports = {
  tabToChordPro: (tab, artist, title, capo, key, callback) => {
    ...
    for (const line of lines) {
      ...
      if (CHORD_LINE_REGEX.test(line) && !TAB_LINE_REGEX.test(line)) {
        ...
        for (const chord of words) {
          ...
          if (hasNext) {
            ...
            if (nextLine.length !== 0 && nextLine.length > chordIndex) {
              const nextChordIndexOrEnd =
                words.length <= wordIndex + 1
                  ? nextLine.length
                  : line.indexOf(words[wordIndex + 1], position);

              const lyric = nextLine.substr(
                chordIndex,
                nextChordIndexOrEnd - chordIndex
              );
        ...
        if (
          hasNext &&
          (TAB_LINE_REGEX.test(lines[lineIndex + 1]) ||
            (hasNextNext && TAB_LINE_REGEX.test(lines[lineIndex + 2])))
        ) {
          part = "tab";
        } else {
